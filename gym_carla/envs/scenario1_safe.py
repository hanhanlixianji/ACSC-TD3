...
class CarlaEnv(gym.Env):
  ...
  def __init__(self, params):
    ...
    observation_space_dict = {
        # create a obs_size*obs_size*3 array, each element value range is (0,255)
      'camera': spaces.Box(low=0, high=255, shape=(self.obs_size, self.obs_size, 3), dtype=np.uint8),
      'lidar': spaces.Box(low=0, high=255, shape=(self.obs_size, self.obs_size, 3), dtype=np.uint8),
      'birdeye': spaces.Box(low=0, high=255, shape=(self.obs_size, self.obs_size, 3), dtype=np.uint8),
      'state': spaces.Box(np.array([-2, -1, -5, 0]), np.array([2, 1, 30, 1]), dtype=np.float32) # STATE shape is (4,) 
                                                                                                # 车辆的当前状态分别为:
                                                                                                #与车道线的横向距离，与车道线的夹角，当前速度，和前方车辆的距离
      }
    if self.pixor:
      observation_space_dict.update({ # 把下边的新字典合并到上边原有的
  ...
  def reset(self):
    ...
    while True:
      ...
      if self.task_mode == 'roundabout':
        ...
        self.start=[13, 306.5, 0.5, 0] # static
    ...
    self.prev_loc = get_pos(self.ego)
    self.stuck_num = 0
  ...
  def convert_action_interval(self, action, before_min=-1, before_max=1, after_min=0, after_max=1): # (-1,1)->>(0,1)
      action = ((action - before_min)/(before_max - before_min)) * (after_max - after_min) + after_min
      return action
  ...
  def step(self, action):
    ...
    #     brake = 0.3
    # else:
    #     throttle = self.convert_action_interval(acc/3)
    #     brake = 0
      # throttle = self.convert_action_interval(acc/3)          ################################
      # brake = 0
    ...
    # if int(vehicle_front) == 1:
    #     steer = 0
    #     throttle = 0
    #     brake = 1
    # else:
    #     throttle = self.convert_action_interval(action[0]/3)
    #     steer = action[1]
    #     brake = 0
    ...
    online_loc = get_pos(self.ego)
    
    # if online_loc == self.prev_loc:
    #     self.stuck_num += 1
    ...
    if info['vehicle_front'] == False:
        if np.sqrt((online_loc[0]-self.prev_loc[0])**2+(online_loc[1]-self.prev_loc[1])**2) <= 0.01:
            self.stuck_num +=1
    self.prev_loc = online_loc
    ...
    return (self._get_obs(), self._get_reward(), self._terminal(), self.success, self.accident, copy.deepcopy(info))
  ...
  #   (self.display_size * 2, self.display_size), # 长 256*3，宽 256
  #   pygame.HWSURFACE | pygame.DOUBLEBUF)

  #   pixels_per_meter = self.display_size / self.obs_range # 8
  #   pixels_ahead_vehicle = (self.obs_range/2 - self.d_behind) * pixels_per_meter # 32
  #   birdeye_params = {
  ...
  def _get_reward(self):
    ...
    r_brake = 0
    brake = self.ego.get_control().brake
    if brake:
        r_brake = -1
        
    r_throttle = 0
    throttle = self.ego.get_control().throttle
    if throttle:
        r_throttle = 1
    ...
    # r = 100*r_collision + lspeed_lon + r_fast + 100*r_out + r_steer*5 + 0.2*r_lat
    # r = r_collision + lspeed_lon + r_fast + r_out + r_steer + r_lat + 0.1*r_speed                     ##################################
    r = 200*r_collision + lspeed_lon + r_fast + 200*r_out + r_brake + 0.2*r_lat + r_steer + 0.1*r_throttle
  ...
  def _terminal(self):
    ...
    if self.dest is not None:
        if np.sqrt((ego_x-self.dest[0])**2+(ego_y-self.dest[1])**2)<4:
            self.success += 1
            return True
